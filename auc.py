...
def handle_callback_query_back(call):
    ...
    bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id,
                          text="Хорошо❗\nСейчас активных пользователей: {active_users} из 15\nВыбери направление:",
...
@bot.callback_query_handler(func=lambda call: call.data == "prem")
def handle_callback_query_prem(call):
    chat_id = call.message.chat.id
    # Create an inline keyboard with weapon options
    markup = types.InlineKeyboardMarkup(row_width=1)
    button_a83 = types.InlineKeyboardButton(text="Премиум на 1 день", callback_data="a83")
    button_a84 = types.InlineKeyboardButton(text="Премиум на 3 дня", callback_data="a84")
    button_a85 = types.InlineKeyboardButton(text="Премиум на 7 дней", callback_data="a85")
    button_a86 = types.InlineKeyboardButton(text="Премиум на 30 дней", callback_data="a86")
    button_a87 = types.InlineKeyboardButton(text="Премиум на 90 дней", callback_data="a87")
    button_a88 = types.InlineKeyboardButton(text="Премиум на 180 дней", callback_data="a88")


    markup.add(button_a83, button_a84, button_a85, button_a86, button_a87, button_a88)
    bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Хорошо! Выбери нужный премиум:",
                          reply_markup=markup)
...
def handle_callback_query_weapon_master_weapon(call):
    ...
    button_a89 = types.InlineKeyboardButton(text="AUG A3", callback_data="a89")
    button_back = types.InlineKeyboardButton(text="Назад⬇️", callback_data="back")

    markup.add(button_a23, button_a24, button_a25, button_a26, button_a27, button_a28, button_a29, button_a30,
               button_a38, button_a71, button_a72, button_a73, button_a74, button_a89, button_back)
    bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Хорошо! Выбери оружие:",
                          reply_markup=markup)
...
def handle_callback_query_item(call):
    ...
    item_id = {
        ...
        "a80": ("j56d6"),
        "a82": ("wj7v3"),
        "a83": ("Premium for 1 day"),
        "a84": ("Premium for 3 days"),
        "a85": ("Premium for 7 days"),
        "a86": ("2opw0"),
        "a87": ("3gv2z"),
        "a88": ("7l9d3"),
        "a89": ("1r7g1"),
...
def handle_callback_query_grav(call):
    ...
    button_gant = types.InlineKeyboardButton(text="Гантель", callback_data="a90")
    button_back = types.InlineKeyboardButton(text="Назад⬇️", callback_data="back")
    markup.add(button_proto, button_roza, button_nt, button_kr, button_br, button_temn, button_ost, button_prim, button_gant, button_back)
    bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id,
                          text="Хорошо! Теперь выберете  артефакт:", reply_markup=markup)
...
@bot.callback_query_handler(
    func=lambda call: call.data in ["a38", "a39", "a41", "a42", "a43", "a44", "a45", "a46", "a47", "a48", "a49", "a50", "a51", "a52", "a53", "a54", "a55", "a56", "a57", "a58", "a59", "60", "a61", "a62", "a63", "a64", "a65", "a90"])
